...
function createCard(item, id, span_watched, watched, type) {
    ...
    if(item.snippet.description!=="This video is private."){
        var fav = 'favorite_border';
        if (jStorageCheck(id, 'fav')) fav = 'star';
        ...
        var item_template = '' +
            ...
            '<div class="btn-m" data-id="' + id + '"><div class="icon"><img src="yt_icon.png"></div></div>' +
...
function createHtmlCard(data, nextPage, token, clear) {
    ...
    if (favorites !== null) {
        if (JSON.parse(favorites).length > 0 && JSON.parse(favorites)) {
            ...
            $('#favorites').html('<h5 style="line-height: 40px;" class="center">Tap "Mark" button to add post to favorites</h5>');
        }
    } else {
        $('#favorites').html('<h5 style="line-height: 40px;" class="center">Tap "Mark" button to add post to favorites</h5>');
    }
...
function showVideo(id) {
	...
	apiRequest("https://www.googleapis.com/youtube/v3/videos",{
		...
		$('.similar > div').html('' +
			...
			'<div class="btn-m" data-id="' + id + '"><div class="icon"><img src="yt_icon.png"></div></div><img class="activator" src="' + img + '">' +
			'</div></div>' +
			...
			'<i class="material-icons" style="color: #039be5;">' + fav + '</i><span>Mark</span></a>' +
			'</div>' +
			'</div>' +
			'' +
			...
			'style="line-height: 10px;padding-left: 3px;width: 100%;text-transform: uppercase;color: black;">' +
			'<i class="material-icons" style="color: #039be5;">share</i><span>Share</span></a>' +
			'</div>' +
			'</div>' +
			'' +
			'</div>' +
			...
			'<span class="date_published">Published: ' + date + '</span> ' +
			'<p style="margin-top: 10px;" id="description-text"></p>' +
			'<div class="more-text" style="display:none;">More...</div>' +
			'</div> </div> ' +
			'' +
			'<div class="card similar-video" style="">' +
			'<h5 class="center-align" style="color: #6A6A6A;text-transform: uppercase;margin-top: 25px;">Similar videos</h5>' +
